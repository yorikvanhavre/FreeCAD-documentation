...
FreeCAD es un modelador 3D [ CAD ](http://en.wikipedia.org/wiki/CAD) paramétrico de propósito general. El desarrollo es completamente [Open Source](http://en.wikipedia.org/wiki/Open_source) (Licencia LGPL). FreeCAD is aimed directly at [mechanical engineering](http://en.wikipedia.org/wiki/Mechanical_engineering) and [product design](http://en.wikipedia.org/wiki/Product_design) but also fits in a wider range of uses around engineering, such as architecture or other engineering specialties.
