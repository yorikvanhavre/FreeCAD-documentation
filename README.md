...
5. [ ] Define a strategy for migration. Auto conversion scripts? wiki <-> md is easy ([pandoc](http://pandoc.org/), other [scripts](https://github.com/Gozala/markdown-wiki) etc). Translations is tricky. There is also the question of images. There are complete [migration tools](https://github.com/philipashlock/mediawiki-to-markdown) too. Quick way: 1) Go to http://www.freecadweb.org/wiki/Special:Export/Arch_Wall 2) save and reupload images 3) `pandoc --from mediawiki --to markdown Arch_Wall.xml` 4) Create a new file, paste, fix links
